...
ClearPost `0.1.0` has no analytics, telemetry, advertising, account system, or developer-operated server.
...
The content script can see the X page DOM because that is required to identify the active composer. Its network-facing message contains only the captured composer text.
...
ClearPost does not persist submitted text, model responses, or a checking history. Removing the extension removes its extension-local storage under normal Chromium behavior.
