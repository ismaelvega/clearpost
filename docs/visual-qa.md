...
| Overlay anatomy | Underlined wordmark, loading line, correction rows, metadata, copy/dismiss actions | Production Shadow DOM render contains each state and preserves the 392px compact width | Matched. The implementation sits at the true bottom-right instead of being centered on a presentation board. |
